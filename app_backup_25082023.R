...
# Server logic ------------------------------------------------------------
    ...
    my_check <- function(){
        if(status() != "yes") { 
            shinyalert::shinyalert(title = "Не выполнен вход", 
                                   text = 'Вносить записи можно только после авторизации. Получите пароль у <a href="https://t.me/faunistica_2_bot" target="_blank">телеграм-бота</a> и введите его.',
                                   type = "error",
                                   html = TRUE)
            return("problem")
        } else { 
            errors <- character()
            if(nchar(input$country) < 5) {errors <- c(errors, "Страна указана не корректно")}
            if(nchar(input$region) < 5)  {errors <- c(errors, "Регион указан не корректно")}
            if(nchar(input$district) < 5){errors <- c(errors, "Район указан не корректно")}
            
            if(nchar(input$NN) == 0){
                errors <- c(errors, nn1 = "Широта не заполнена")
            } else { 
                if(str_detect(input$NN, "[:alpha:]")) {errors <- c(errors, nn2 = "Буквы в координатах широты")}
                if(str_detect(str_replace_all(input$NN, "\\,|\\.", ""), "[:punct:]|[:symbol:]|[:space:]")) { 
                    errors <- c(errors, nn3 = "Недопустимые символы в координатах широты")}
                if((str_count(input$NN, "\\.") + str_count(input$NN, "\\,")) > 1) {
                    errors <- c(errors, nn4 = "Слишком много разделителей (точек и/или запятых) в широте")}
                if(str_count(input$NN, "[:digit:]") < 3){errors <- c(errors, nn5 = "Недостаточная точность широты_1")}
            }
            if(str_detect(paste0(names(errors), collapse = ""), "nn", negate = TRUE)){ 
                if(str_detect(input$NN, "\\.|\\,", negate = TRUE)) {
                    errors <- c(errors, "Недостаточная точность широты_2")
                } else if(nchar(str_split_1(str_replace(input$NN, "\\,", "."), "\\.")[[2]])<2) { 
                    errors <- c(errors, "Недостаточная точность широты_3")}
                nn <- as.numeric(str_replace(input$NN, "\\,", "."))
                if(nn < 50 | nn > 71) {errors <- c(errors, "Точка выходит за границы исследуемого региона по широте")}}
            }
            
        if(nchar(input$EE) == 0){
            errors <- c(errors, ee1 = "__ не заполнена")
        } else { 
            if(str_detect(input$EE, "[:alpha:]")) {errors <- c(errors, ee2 = "Буквы в координатах долготы")}
            if(str_detect(str_replace_all(input$EE, "\\,|\\.", ""), "[:punct:]|[:symbol:]|[:space:]")) { 
                errors <- c(errors, ee3 = "Недопустимые символы в координатах __")}
            if((str_count(input$EE, "\\.") + str_count(input$EE, "\\,")) > 1) {
                errors <- c(errors, ee4 = "Слишком много разделителей (точек и/или запятых) в долготе")}
            if(str_count(input$EE, "[:digit:]") < 3){errors <- c(errors, ee5 = "Недостаточная точность долготы_1")}
        }
        if(str_detect(paste0(names(errors), collapse = ""), "ee", negate = TRUE)){ 
            if(str_detect(input$EE, "\\.|\\,", negate = TRUE)) {
                errors <- c(errors, "Недостаточная точность долготы_2")
            } else if(nchar(str_split_1(str_replace(input$EE, "\\,", "."), "\\.")[[2]])<2) { 
                errors <- c(errors, "Недостаточная точность долготы_3")}
            ee <- as.numeric(str_replace(input$EE, "\\,", "."))
            if(ee < 50 | ee > 75) {errors <- c(errors, "Точка выходит за границы исследуемого региона по долготе")}}
...
# PAGE input new data: auth ----------------------------------------------------
    output$i_auth <- renderUI(tagList(
        ...
        fluidRow(
            ...
            column(width = 3, 
                column(width = 6, actionButton("auth", 
                                               # "in",
                                               r("i_auth.in"),
                                               width = '80%', 
                                               style="float:left")), # width = '100%'
                column(width = 6, actionButton("deauth", 
                                               # "out",
                                               r("i_auth.out"),
                                               # disabled = TRUE, 
                                               width = '80%', 
